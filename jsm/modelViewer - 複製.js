...
import { OrbitControls } from "./threejs/controllers/OrbitControls.js";
...
class ModelViewer {
	...
	renderMeshAsync = (xyzDims, ratio, id, scene) => {
		...
		return new Promise(
			function (resolve, reject) {
				_this.renderMesh(xyzDims, ratio, id, scene)
				resolve()
			}
		)
	}
	...
	renderMesh = function (xyzDims, ratio, id, scene) {
		...
		if (group == null) {
			...
			material.clippingPlanes = group.material.clippingPlanes
