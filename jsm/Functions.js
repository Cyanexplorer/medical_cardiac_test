...
class Morphology {
	constructor() {
		...
		this.erode = (dims, dataBuffer, paddingSize) => {
			...
			edge(dataBuffer, dims, kernel, kDims, paddingSize)
		}
		...
		this.open = (dims, dataBuffer, paddingSize) => {
			this.erode(dims, dataBuffer, paddingSize)
			this.dilate(dims, dataBuffer, paddingSize)
		}
		...
		this.medium = (dims, dataBuffer, paddingSize) => {
			let kernelSize = paddingSize * 2 + 1
			let kernel = new Uint8Array(kernelSize * kernelSize * kernelSize).fill(255)
			let kDims = [kernelSize, kernelSize, kernelSize]

			blur(dataBuffer, dims, kernel, kDims, paddingSize)
		}

		this.gaussian = (dims, dataBuffer, paddingSize) => {
			let ksize = paddingSize * 2 + 1
			let kernel = new Float32Array(ksize * ksize * ksize).fill(0)
			...
			for (let i = 0; i < ksize; i++) {
				...
				for (let j = 0; j < ksize; j++) {
					...
					let g = Math.exp(-(x2 + y2) / (2 * sigma * sigma));
					g /= 2 * Math.PI * sigma;
					kernel[pos++] = g;
				}
			}
		...
		let blur = (dataBuffer, dims, kernel, kDims, paddingSize) => {
			console.log(kernel)
			let paddingBuffer = padding(dims, dataBuffer, paddingSize)
			let tmp = new Uint8Array(dataBuffer)
				...
				for (let i = 0; i < kDims[2]; i++) {
					for (let j = 0; j < kDims[1]; j++) {
						for (let k = 0; k < kDims[0]; k++) {
							p += kernel[kpos] * paddingBuffer[getPosition(x + k, y + j, z + i, paddingSize, false)]
							sum += kernel[kpos]
							kpos++
						}
					}
			...
			for (let i = 0; i < dims[2]; i++) {
				for (let j = 0; j < dims[1]; j++) {
					for (let k = 0; k < dims[0]; k++) {
						if (tmp[pos] == kCenter) {
							dataBuffer[pos] = calculate(k, j, i)
						}
						pos++
					}
				}
		...
		let edge = (dataBuffer, dims, kernel, kDims, paddingSize) => {
...
class RegionGrowing {
	static process = (x, y, layerIndex, source, dims, bias, margin) => {
		...
		let stack = new Uint16Array((count + 16) * 5)
		let stackSize = 0
