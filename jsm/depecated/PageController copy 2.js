...
class ImageDataUploader {
	constructor() {
		...
		let mapCLAHE = (dataBuffer, max, dims) => {
			let blockSize = 128
			let header = dims[0] * dims[1]
			let body = dims[0]
			let climit = dataBuffer.length * 0.4
			...
			let cdfs = new Array(tiles[2])
			for (let i = 0; i < tiles[2]; i++) {
				cdfs[i] = new Array(tiles[1])
				for (let j = 0; j < tiles[1]; j++) {
					cdfs[i][j] = new Array(tiles[0])
				}
			}
			...
			for (i = 0, istep = 0; i < dims[2]; i += blockSize, istep++) {
				...
				for (j = 0, jstep = 0; j < dims[1]; j += blockSize, jstep++) {
					...
					for (k = 0, kstep = 0; k < dims[0]; k += blockSize, kstep++) {
						...
						cdfs[istep][jstep][kstep] = buildcdf(k, j, i, kDims)
					}
				}
			}
			...
			for (i = 0; i < dims[2]; i++) {
				...
				for (j = 0; j < dims[1]; j++) {
					...
					for (k = 0; k < dims[0]; k++) {
						...
						let cdf0 = cdfs[zmin][ymin][xmin][pixel]
						let cdf1 = cdfs[zmin][ymin][xmax][pixel]
						let cdf2 = cdfs[zmin][ymax][xmin][pixel]
						let cdf3 = cdfs[zmin][ymax][xmax][pixel]
						let cdf4 = cdfs[zmax][ymin][xmin][pixel]
						let cdf5 = cdfs[zmax][ymin][xmax][pixel]
						let cdf6 = cdfs[zmax][ymax][xmin][pixel]
						let cdf7 = cdfs[zmax][ymax][xmax][pixel]
