...
class ScaleField {
	...
	constructor(width, height, depth, dataBuffer, rgba) {
		...
		this.volumeData = [new Float32Array(dataBuffer), new Float32Array(arraySize), new Float32Array(arraySize)]

		this.t = new Float32Array(arraySize)
		this.sizeData = new Uint8Array(arraySize)
		this.alpha = dataBuffer
	...
	process(onprogress, onload) {
		...
		gaussian1.diff(this, (volume, progress) => {
			...
			if (progress == 1) {
				...
				this.alpha = this.sizeData

				for(let i= 0 ;i<this.sizeData.length;i++){
					if(this.alpha[i]>0){
						this.alpha[i] = this.sizeData[i]
					}
				}
			...
			for (let i = 0; i < data.dims[2]; i++) {
				for (let j = 0; j < data.dims[1]; j++) {
					for (let k = 0; k < data.dims[0]; k++) {
						...
						//tmpAlpha[index] = data.volumeData[1][index]
						data.t[index] = N - 1
						index++
					}
				}
