...
self.addEventListener('message', (e) => {
    ...
    let calculate = (x, y, z) => {
        x -= parseInt(kDims[0] / 2)
        y -= parseInt(kDims[1] / 2)
        z -= parseInt(kDims[2] / 2)
        ...
        for (let i = 0; i < kOrder.length; i++) {
            ...
            let kPos_x = Math.abs(x + kOrder[i][0])
            let kPos_y = Math.abs(y + kOrder[i][1])
            let kPos_z = Math.abs(z + kOrder[i][2])

            template.setValue3D(target, kPos_x, kPos_y, kPos_z)
        }
    }
    ...
    for (i = 0; i < dims[2]; i++) {
        for (j = 0; j < dims[1]; j++) {
            for (k = 0; k < dims[0]; k++) {
                if (binData.getBit(pos++) != target)
                    continue

                calculate(k, j, i, target)
            }
        }
    }
